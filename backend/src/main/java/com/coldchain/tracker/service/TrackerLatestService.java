...
public class TrackerLatestService {
    ...
    public TrackerLatestUpsertOutcome upsert(String trackerId, Instant recordedAt, BigDecimal temperature, Point position) {
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                if (attempt == MAX_ATTEMPTS) {
                    return TrackerLatestUpsertOutcome.CONFLICT;
                }
