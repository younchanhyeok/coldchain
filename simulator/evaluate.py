...
사용 예:
  python evaluate.py --admin-key <키> --reps 3 --trackers 10 --route-minutes 30 \
...
def main():
    ...
    parser.add_argument("--route-minutes", type=float, default=30.0,
                        help="경로 주파(분) = 페이즈 길이. 기본 30 — 기본 프로파일에 slow-rise(이탈 ~27분)가 "
                             "있어 30 미만이면 slow-rise가 이탈 못 해 전부 FP로 오염된다(L311 가드).")
