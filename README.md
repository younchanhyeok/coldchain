...
| M6 | 스케일 — 부하테스트→Kafka·Timescale 전환 | 개선 전/후 수치 비교 리포트 | 완료 |
| M7 | 예측 심화 — 다변량·평가 자동화 | v1 vs v2 모델 수치 비교 | 완료 |
| M8 | 서사 완결 — 하드닝·수평확장 능력·예측 벤치 재평가 | v2 재-activate 증명·FP 서사 강화 | 완료 |

### M8 결과 요약 — 재벤치: v2의 급변 재-activate 증명, 그리고 못 넘은 한계 두 개

M7 벤치가 남긴 숙제 3건을 신규 프로파일(slow-rise·gentle-failure)·30분 경로·더 큰 표본으로 다시 쟀다.

- ✅ **급변 후 재-activate 증명(핵심 소득):** 완만한 냉동기 고장(gentle-failure)에서 **v2가 v1과 동일하게 30건 전부 적중, 오탐 0**. M7의 급경사 고장에선 v2가 0적중이었는데, 고장이 윈도우(5점)보다 느리면 regime-trim이 새 국면에 재적합해 이탈을 잡는다. (급경사 고장은 여전히 못 잡음 — 개선의 경계를 정직하게 명시.)
- ✅ **FP 제거 서사 강화:** 오탐률 0.68→**0.04**, normal 189→0·plateau 155→0·slow-rise 128→3. v1의 진짜 소음은 FP보다 **INVALIDATED(~1,900건, 발령→무효화→재발령 루프)** — v2는 그 루프를 억제(13건).
- ⚠️ **못 넘은 한계 2개(숨기지 않음):** (1) 리드타임은 slow-rise로도 여전히 ~0 — 원인은 지표가 무효화 churn 하에서 "이탈 직전 재활성 조각"만 재기 때문(지표 설계 문제로 규명, M9 후보). (2) 시각오차는 기대와 반대로 v2가 열세(0.93 vs 4.48분).

여전히 트레이드오프(v2=오탐0·저민감)라 `PREDICTION_MODEL` 기본값은 v1 유지. 상세는 개발 노트 참조.

### M7 결과 요약 — v2 뉴턴 냉각 모델: 오탐을 없애되 민감도를 내준다 (트레이드오프)

v1(온도-시간 선형회귀)에 외기온을 입력으로 더한 v2(뉴턴 냉각 물리 모델)를 만들고, 같은 시나리오 세트를 모델 토글로 짝지어 3회 실행해 비교했다. 결말을 아는 4개 프로파일(normal·gradual-rise·sudden-failure·plateau)을 각 10 트래커·15분 경로로 주행.

| 지표 (3-rep 평균) | v1-linear | v2-newton |
|---|---|---|
| 오탐률 | 0.67 | **0.03** |
| 적중률 | 0.33 | **0.97** |
| 놓침(missed) | 12.7 | 18.7 |

**핵심 서사 — "구조적 오탐 제거":** v1은 상승 구간만 보고 임계 도달을 외삽해, 임계 바로 아래에서 점근하는 plateau(FP 42→0)와 정상 곡선(FP 84→0)에서 헛경보를 쏟아낸다(발령의 2/3가 오탐). v2는 `ambient ≤ threshold`면 물리적으로 도달 불가로 판정해 이 구조적 FP를 없앤다. **대가는 민감도** — v2는 보수적이라 놓침이 늘고 sudden-failure(냉동기 고장) 급변은 놓쳤다(실측된 약점). 정확도 우위가 아니라 트레이드오프라, 기본값은 v1을 유지하고 v2는 저-오탐 선택지로 env 토글(`PREDICTION_MODEL`)로 남겼다. 재현은 `simulator/evaluate.py`(모델 토글 짝지은 재실행).

### M6 결과 요약 — 5000 트래커(1000 리딩/s)에서 무너진 것을 살리기

부하를 실측해 병목을 겪은 뒤에만 기술을 도입한다는 원칙으로, 값싼 최적화(배치·풀·N+1 제거) → Kafka(수집 큐) → TimescaleDB(hypertable·다운샘플) 순으로 전환했다.

| 지표 (5000 트래커) | baseline | 최종 |
|---|---|---|
| 대시보드 조회 p50 | 22.7초 (83% 타임아웃) | **412ms (0%)** |
| SSE 반영 지연 | 290초 백로그(무한 누적) | 37.8초 랙(유계·복구가능) |
| 수집 202 p99 | 462ms | **90ms** |

**핵심 서사 — "신선하지만 손실 vs 지연되지만 무손실"**: 동기 저장(direct)은 과부하 시 다운스트림 작업 51만 건을 조용히 버려 수집을 지킨다(NFR-3). Kafka는 버리는 대신 브로커에 내구성 있게 쌓아(드롭 0) 무손실을 지키고, 초과 부하는 수평 확장으로 해소 가능한 "빚"으로 만든다. 단일 인스턴스 실시간 소비 용량 ≈ 트래커 3,000개, 5,000개는 수신 무손실. 상세 수치·서사는 개발 노트 참조.
