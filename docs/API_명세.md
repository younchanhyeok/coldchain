...
// 200
{
  ...
  "hitRate": 0.776,            // 적중률 = TP/(TP+FP)
  "avgLeadTimeMinutes": 11.3,  // breachedAt-createdAt 평균(이탈 몇 분 전에 최초 경고했나)
  "medianLeadTimeMinutes": 9.0,
  "episodes": [
    { "trackerId": "TRK-0001", "productName": "백신 A (gradual-rise)", "status": "BREACHED", "leadTimeMinutes": 12 }
  ]
}
```
`episodes[]`는 리포트 탭 "시나리오 결과 테이블"의 데이터원 — 기간 내 생성된 예측 에피소드 전부(진행 중인 `ACTIVE`도 포함되되 TP/FP 집계에는 안 들어간다). `totalPredictions`는 이 기간에 **생성**된 에피소드 수 기준(적중·오탐 여부와 무관), TP/FP/리드타임은 그중 **종결된** 것만 집계한다. "정확도를 자랑하는 게 아니라 측정한다"의 API 표면 — 어드민 화면(v2)과 M7 비교 리포트가 이 위에 얹힘.
