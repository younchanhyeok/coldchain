...
// 200
{
  ...
  "avgBreachTimingErrorMinutes": 2.4,  // |예측 시각-실제 이탈 시각| 평균(BREACHED 한정, M7) —
                                        // "얼마나 정확히 맞췄나". 리드타임(얼마나 앞섰나)과 별개 축.
  "episodes": [
...
### 평가 런 — POST/GET /api/v1/admin/evaluation-runs (M7)
위 온디맨드 지표를 **"런" 단위로 상시 기록**한 스냅샷. 지표 수학은 `getMetrics` 그대로 재사용하고, 기간·모델버전·라벨이라는 런 정체성을 고정한다(episodes는 저장 안 함 — 라이브 재조회 가능). v1 vs v2 비교·추이의 데이터원.
- **`@Scheduled`(매시 정각-1h 창)**: 존재하는 modelVersion별 1건씩 SCHEDULED 런. 에피소드 0건 창은 스킵, 같은 (기간,버전) 런은 부분 유니크 인덱스로 멱등.
- **`POST`** (X-Admin-Key): body `{from, to, label?, modelVersion?}` — 비교 스크립트가 페이즈별 라벨로 수동 스냅샷. modelVersion 생략 시 전체 합산. 응답은 스냅샷된 런(위 지표 필드 + `label`/`triggerType`/`createdAt`).
- **`GET ?limit=`** (X-Admin-Key, 기본 50·최대 200): 최근 런 목록(createdAt 내림차순). 리포트 뷰의 런 셀렉터·v1/v2 비교 데이터.
