...
// 실패·재시도)만 표시한다.
export function AlertsKpiTiles({ alerts }: AlertsKpiTilesProps) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
      <KpiTile icon={AlertTriangle} tone="primary" label="오늘 발생" value={String(todayCount)} sub="오늘 기록된 알림" hasValue />
      <KpiTile icon={CheckCircle2} tone="success" label="발송 성공" value={String(sentCount)} sub="SENT 상태" hasValue />
      <KpiTile
        icon={Send}
        tone="danger"
        label="발송 실패·재시도"
        value={String(failedCount)}
        sub="FAILED 상태(재시도 소진 포함)"
        hasValue
      />
    </div>
  )
