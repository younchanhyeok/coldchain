...
- **복잡도는 핵심 도메인에만.** 이 시스템의 핵심 복잡도는 예측(L3)·이상탐지(L2)·시계열/공간 쿼리에 있다. 수집·알림·SSE 같은 주변 모듈은 검증된 단순한 패턴으로 유지하고, 요구사항에 없는 추상화·범용화를 더하지 않는다 (YAGNI).
...
- M6~: 수집 경로 기본이 Kafka다 — 브로커가 안 떠 있으면 수집이 503(`INGEST_UNAVAILABLE`). 브로커 없이 돌리려면 `INGEST_MODE=direct`(M1~M5 동기 저장, A/B 비교 토글).
